...
4. **Assess strategic fit** — Does this align with the user's solo / native-mobile / AI-assisted positioning, or pull in a different direction?
...
the user's operation is solo and bounded. Every Spec consumes one or more of these scarce resources:
...
- **Attention cost**: the user's mental bandwidth. New vocabulary, new flags, new state files — all increase context-switch overhead.
...
- **Differentiation**: Does this Spec strengthen, weaken, or leave neutral the user's competitive position in his target market (Apple-native productivity, AI-assisted solo dev)?
...
- Strategic focus diverges from the named target market (e.g., Spec optimizes web-SaaS workflow when the user ships native iOS)
...
- **Bon-only knowledge**: Is the Spec creating tacit knowledge only the user can debug? Solo dev is OK with this, but recurring patterns of "the user is the only one who understands X" compound over time.
...
Specific to the user's Pro/Max tier with Opus 4.7:
...
- **Hook frequency cost**: Hooks running on every turn / every tool call / every Stop. Estimate calls-per-day at the user's usage pattern.
- **Plan limit risk**: Could this Spec push the user over weekly limits on Pro/Max tier?

If token cost is ambiguous, recommend a back-of-envelope calculation be added to the Spec.

### 6. Strategic Alignment (MEDIUM)

- **the user's stated direction**: Solo dev, native mobile, AI-assisted, Apple-native productivity. Does this Spec move in this direction or orthogonal to it?
- **Time allocation**: How many hours of the user's attention does implementation + maintenance take? Is this the highest-leverage use of those hours?
...
- **Indirect revenue link**: Does this enable the user to ship more revenue work?
